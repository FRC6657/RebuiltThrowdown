...
import frc.robot.subsystems.drive.DrivebaseConstants.DriveConstants.SwerveModuleInformation;
import frc.robot.subsystems.drive.DrivebaseConstants.MAXSwerveConstants;
import org.littletonrobotics.junction.Logger;

public class MAXSwerveIO_Relative implements MAXSwerveIO {
  ...
  public MAXSwerveIO_Relative(SwerveModuleInformation moduleInformation) {
    ...
    driveConfig.closedLoop.feedForward.kV(1d / MAXSwerveConstants.kMaxDriveSpeed);
    ...
    turnConfig.closedLoop.feedForward.kV(MAXSwerveConstants.kTurnFF);
    turnConfig.closedLoop.positionWrappingEnabled(false);
    turnConfig.closedLoop.outputRange(
        MAXSwerveConstants.kTurnMinOutput, MAXSwerveConstants.kTurnMaxOutput);

    turnConfig.encoder.positionConversionFactor((203d / 9424d) * (2 * Math.PI));
    turnConfig.encoder.velocityConversionFactor((203d / 9424d) * (2 * Math.PI) * (1d / 60d));
  ...
  public void setDriveMPS(double mps) {
    driveSetpoint = mps;
    drivePID.setSetpoint(mps, ControlType.kVelocity);
  }
